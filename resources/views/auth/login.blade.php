...
<div class="flex flex-col items-center justify-center min-h-[70vh]">
    <div class="w-full max-w-md bg-white/90 shadow-xl rounded-2xl px-10 py-8 border border-blue-100 animate-fade-in">
        <h2 class="text-3xl font-extrabold mb-8 text-center text-blue-700 tracking-tight">تسجيل الدخول</h2>
        <form method="POST" action="{{ url('/login') }}" class="space-y-6">
            @csrf
            <div>
                <label for="email" class="block mb-2 text-sm font-semibold text-gray-700">البريد الإلكتروني</label>
            ...
            <div>
                <label for="password" class="block mb-2 text-sm font-semibold text-gray-700">كلمة المرور</label>
            ...
            <div class="flex items-center justify-between">
                <label class="inline-flex items-center select-none">
                    ...
                    <span class="mr-2 text-gray-600 text-sm">تذكرني</span>
                </label>
                <a href="{{ route('register') }}" class="text-blue-600 hover:underline text-sm font-medium">مستخدم جديد؟ سجل الآن</a>
            </div>
            <button type="submit" class="w-full py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-lg shadow transition-all duration-150 focus:outline-none focus:ring-2 focus:ring-blue-200">دخول</button>
        </form>
    </div>
</div>
@endsection
