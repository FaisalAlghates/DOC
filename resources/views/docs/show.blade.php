...
<div x-data="{ tab: '{{ ($doc->documentation && $doc->documentation->doc_type === 'bestpractice') || (isset($doc->doc_type) && $doc->doc_type === 'bestpractice') ? 2 : 1 }}' }" class="max-w-4xl mx-auto py-12 px-2 md:px-6">
    ...
    <div class="bg-white/95 rounded-3xl shadow-2xl border border-blue-100 p-0 md:p-10 flex flex-col gap-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center gap-4 md:gap-8 px-6 pt-8">
            ...
            <div class="flex-1">
                <h2 class="text-3xl md:text-4xl font-extrabold text-blue-700 mb-1">{{ $doc->documentation->title ?? $doc->title ?? 'Title not specified' }}</h2>
        ...
        <div class="mx-6 mb-6 p-4 bg-gray-50 rounded-xl border border-gray-200">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <span class="font-semibold text-gray-700">Last edited by:</span>
                    <span class="text-gray-600">{{ $lastHistory->user->name ?? 'Not specified' }}</span>
                </div>
                <div class="flex items-center gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-purple-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    <span class="font-semibold text-gray-700">Last modification date:</span>
                ...
                <div class="md:col-span-2 flex items-center gap-2">
                    ...
                    <span class="text-gray-500 italic">This documentation has not been modified yet</span>
                </div>
                @endif
            </div>
        </div>
        <div x-show="tab == 1">
            ...
            <div class="grid grid-cols-1 gap-8 px-6">
                <div class="flex flex-col gap-4 bg-gray-50/60 rounded-xl p-6 border border-gray-200">
                    <div class="text-lg font-bold text-gray-700 mb-2">Testing</div>
                    @foreach($testings as $test)
                        <div class="mb-2">
                            <div class="font-semibold text-gray-800">Test Type: <span class="font-normal">{{ $test->test_type }}</span></div>
                            <div class="text-gray-700">Description: {{ $test->test_description }}</div>
                            @if($test->test_results)
                                <div class="text-green-700">Results: {{ $test->test_results }}</div>
                            @endif
                        </div>
                    @endforeach
                </div>
